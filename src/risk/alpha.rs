...
impl AlphaEngine {
    pub fn new(confidence: f64) -> Self {
        let ml_runner = OnnxRunner::new("d:/Deriv/model_v3.onnx")
            .map_err(|e| {
                tracing::warn!("Failed to load ONNX model v3 (fallback to quant model): {}", e);
                e
            })
            .ok();
    ...
    pub fn update_spot_return(&mut self, price: f64, ts: UnixMs) {
        ...
        if self.prices.len() > 1000 {
            // We need at least 50 for vol_heat
    ...
    pub fn calculate_q_model(&self, s0: f64, s_hat: f64, time_left_sec: f64) -> Prob {
        ...
        if let Some(ref runner) = self.ml_runner {
            if self.prices.len() >= 50 {
                ...
                // 3. RSI 14 (Simple version for speed)
                let rsi_14 = {
                    let mut gains = 0.0;
                    let mut losses = 0.0;
                    for i in 0..14 {
                ...
                // 8. Volatility Heat
                let vol_heat = {
                    let std_10 = ((0..10).map(|i| (p(i) - sma_10).powi(2)).sum::<f64>() / 10.0).sqrt();
                    let sma_50: f64 = (0..50).map(|i| p(i)).sum::<f64>() / 50.0;
                    let std_50 = ((0..50).map(|i| (p(i) - sma_50).powi(2)).sum::<f64>() / 50.0).sqrt();
                    std_10 / std_50.max(1e-9)
                };

                let features = [
                    ret as f32, dist_sma10 as f32, dist_sma30 as f32,
                    rsi_14 as f32, bb_position as f32, bb_width as f32,
                    vol_20 as f32, fractal_efficiency as f32, 
                    mom_accel as f32, vol_heat as f32
                ];
    ...
    /// Shrink q_model towards q_mkt via logit-blend.
...
mod tests {
    ...
    fn test_q_model_at_expiry() {
        let engine = AlphaEngine::new(0.55);
